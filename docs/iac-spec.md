...
| CI - IaC (Terraform) | `.github/workflows/ci-iac.yaml` | `iac/**` |

### 6.1 フロントエンド・バックエンド スキャン内容
...
### 6.2 IaC CI/CD (`ci-iac.yaml`)

#### フロー概要

```
pull_request (iac/** 変更あり)
  └─ fmt → Analyze × 3 (静的解析ゲート) → Plan × 3 → PR コメント → Slack

push to main (iac/** 変更あり)
  └─ fmt → Analyze × 3 (静的解析ゲート) → Apply × 3 (iac-apply 承認) → Slack

workflow_dispatch
  └─ fmt → Analyze × 3 → Slack (静的解析のみ)
```
...
| 項目 | 内容 |
|---|---|
| トリガー | `pull_request` to `main`、`iac/**` または `ci-iac.yaml` に変更あり |
| 前提条件 | `fmt` + `Analyze` 全クラウド通過後に実行 |
| OIDC 認証 | plan 用 ID (読み取り専用) を使用 |
| 結果通知 | PR コメントに `Plan: X to add, Y to change, Z to destroy.` のみ表示 (アカウント ID 等は非表示) |
| 並列制御 | 同一ブランチ・同一クラウドの古い plan はキャンセル (`cancel-in-progress: true`) |

#### apply (main マージ時)

| 項目 | 内容 |
|---|---|
| トリガー | `push` to `main`、`iac/**` または `ci-iac.yaml` に変更あり |
| 前提条件 | `fmt` + `Analyze` 全クラウド通過後に実行 |
| 承認ゲート | GitHub Environment **`iac-apply`** の必須レビュアー承認が必要 |
| OIDC 認証 | apply 用 ID (全権) を使用 |
| apply 方式 | `terraform plan -out=tfplan` → `terraform apply tfplan` で PR 時の差分と乖離させない |
| 並列制御 | クラウドごとに直列化 (`cancel-in-progress: false`)、state ロック競合を防ぐ |

#### plan/apply 共通

| 項目 | 内容 |
|---|---|
| state backend | `-backend-config` で GitHub Variables (`TF_STATE_*`) から接続先を注入 |
| tfvars 注入 | ハイフンを含む変数名 (`github-repository-name` 等) は `ci.auto.tfvars` を CI で生成して渡す |
| auth0 認証情報 | `TF_VAR_auth0_*` GitHub Secrets として注入 |

#### plan/apply の OIDC 権限分離

| ロール | 権限 | 利用できる OIDC subject |
|---|---|---|
...
apply ロールは `iac-apply` Environment の**必須レビュアー承認**を経なければ取得できない subject からしか使えない。

#### 承認運用

1. `iac/**` の変更を含む PR を main にマージ
2. apply ワークフローが起動し、**`iac-apply` Environment の承認待ち**状態になる
3. 必須レビュアー (jinka1997) が内容を確認し承認 → `terraform apply` が実行される
4. 承認しない場合はタイムアウト (デフォルト 30 日) またはキャンセルで apply をスキップできる

> **重要:** apply は必ず事前に PR の plan 結果を確認してから承認すること。

#### セキュリティ上の注意

パブリックリポジトリのため Actions タブ・Summary は誰でも閲覧可能。
`terraform plan` のフル出力 (アカウント ID / ARN / リソース ID 等を含む) は
PR コメントにも Actions Summary にも書き込まない。
詳細差分の確認はローカルで `terraform plan` を実行すること。
...
| ジョブ | 内容 |
|---|---|
| `terraform fmt` | `terraform fmt -check -recursive iac/` — 全クラウドの .tf ファイルを一括フォーマットチェック |
| `Analyze (aws)` | terraform validate + tflint + Trivy misconfig を AWS ディレクトリに実行 |
| `Analyze (azure)` | 同上 (Azure) |
| `Analyze (gcp)` | 同上 (GCP) |
| `notify-slack` | Slack 通知 |

#### terraform validate

`terraform init -backend=false` でプロバイダバイナリのみ取得（クラウド認証不要）してから `terraform validate` を実行する。`.terraform.lock.hcl` が `windows_amd64` のみの場合、init 時に linux_amd64 向けバイナリが追加取得される。

#### tflint

各クラウドに `.tflint.hcl` を配置し、プロバイダ固有のルールセットを有効化する。

| クラウド | .tflint.hcl | プラグイン |
|---|---|---|
| AWS | `iac/aws/.tflint.hcl` | `tflint-ruleset-aws` |
| Azure | `iac/azure/.tflint.hcl` | `tflint-ruleset-azurerm` |
| GCP | `iac/gcp/.tflint.hcl` | `tflint-ruleset-google` |

#### Trivy misconfig（設定ミス検出）

`trivy fs --scanners misconfig` で Terraform の設定ミス（暗号化無効・パブリックアクセス許可・IAM 過剰権限等）を検出する。

**ゲート方針: CRITICAL/HIGH を gate 化 (`exit-code: 1`)**

| 重要度 | 挙動 |
|---|---|
| CRITICAL / HIGH | exit-code 1 → PR マージ不可 |
| MEDIUM / LOW | 表示のみ（non-blocking） |

既知の意図的な設定（テンプレートの PoC 設定等）を抑制する場合は、各クラウドディレクトリに `.trivyignore` を作成してチェック ID を記載する。

```
# iac/aws/.trivyignore の例
AVD-AWS-0001  # S3 public access block disabled — intentional for static site hosting
```

#### PR 必須チェック（jinka1997 が Settings で設定）
...
#### tflint プラグインバージョン更新手順

`.tflint.hcl` のバージョンを変更し `tflint --init` を再実行する（`.tflint.hcl.lock` は `.gitignore` 管理のためコミット不要）。

### 6.3 CVE 抑制 (.trivyignore)
